...
def affine_backward(dout, cache):
    ...
    db = dout.sum(axis=0)               # Sum across batch
        ...
        for c in range(C):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    ...
                    out[n, c, h_out, w_out] = np.max(pool_region)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = (x, x_norm, mean, var, gamma, beta, G, eps)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x, x_norm, mean, var, gamma, beta, G, eps = cache
    N, C, H, W = dout.shape
    ...
    dvar = -0.5 * np.sum(dx_norm_reshaped * (x_reshaped - mean), axis=(2, 3, 4), keepdims=True) * np.power(var + eps, -1.5)
    dmean = -np.sum(dx_norm_reshaped / np.sqrt(var + eps), axis=(2, 3, 4), keepdims=True) - 2 * dvar * np.mean(x_reshaped - mean, axis=(2, 3, 4), keepdims=True)
    
    dx_reshaped = dx_norm_reshaped / np.sqrt(var + eps) + dvar * 2 * (x_reshaped - mean) / group_size + dmean / group_size
