...
try:
    ...
    mlflow.set_experiment("IEEE-CIS-Fraud-Detection_adaboost")
    print(f"MLflow experiment set to: {mlflow.get_experiment_by_name('IEEE-CIS-Fraud-Detection_adaboost').name}")
    mlflow_active = True
...
# Start MLflow run
run_name = f"adaboost_{time.strftime('%Y%m%d_%H%M%S')}"
if mlflow_active:
...
# Define a base estimator (weak learner) for AdaBoost
...
# Create the base estimator with the best parameters
best_base_estimator = DecisionTreeClassifier(
    max_depth=3,
    class_weight=best_params['classifier__base_estimator__class_weight'],
    random_state=42
)
...
if mlflow_active:
    ...
    try:
        # For AdaBoost, feature importances come from the ensemble
